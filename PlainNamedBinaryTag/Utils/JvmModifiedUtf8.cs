    ...
    public static class JvmModifiedUtf8
    {
        ...
        public static string GetString(byte[] bytes)
        {
            ...
            for (int i = 0; i < bytes.Length; i++)
            {
                ...
                {
                    if (i + 2 >= bytes.Length)
                        throw new FormatException($"Expected 3-byte sequence but reached end of input");
                    byte b2 = bytes[++i];
                    byte b3 = bytes[++i];
                    if ((b2 & 0xC0) != 0x80 || (b3 & 0xC0) != 0x80)
                        throw new FormatException($"Continuation bytes 0x{b2:X2} 0x{b3:X2} (at position {i-1}, {i}) do not match 0b_10xx_xxxx pattern");
                    int code = ((b1 & 0x0F) << 12) | ((b2 & 0x3F) << 6) | (b3 & 0x3F);
                    if (code < 0x0800)
                        throw new FormatException($"Overlong encoding for code point 0x{code:X4} (at position {i - 2})");
                    result.Append((char)code);
                }
                else
                {
                    throw new FormatException($"Leading byte 0x{b1:X2} (at position {i}) was not supported");
                }
            }
